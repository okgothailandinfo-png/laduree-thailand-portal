...
function toListItem(row: AdminOrderListRow): AdminOrderListItemDto {
  ...
  return {
    ...
    boutiqueId: order.pickup?.boutiqueId ?? order.delivery?.fulfilmentBoutiqueId ?? '',
    boutiqueName: order.pickup?.boutiqueName ?? 'Delivery',
    pickupDate: order.pickup?.dateKey ?? order.delivery?.dateKey ?? '',
    pickupTime: order.pickup?.timeSlotLabel ?? order.delivery?.timeSlotLabel ?? row.pickupStartTime ?? '',
...
function toDetail(record: AdminOrderDetailRecord): AdminOrderDetailDto {
  ...
  return {
    ...
    boutique: {
      id: order.pickup?.boutiqueId ?? order.delivery?.fulfilmentBoutiqueId ?? '',
      name: order.pickup?.boutiqueName ?? 'Delivery',
      code: record.boutiqueCode,
      address: order.pickup?.address ?? '',
    },
    pickup: {
      date: order.pickup?.dateKey ?? order.delivery?.dateKey ?? '',
      time: order.pickup?.timeSlotLabel ?? order.delivery?.timeSlotLabel ?? record.pickupStartTime ?? '',
      slotId: order.pickup?.timeSlotId ?? order.delivery?.timeSlotId ?? '',
    },
    items,
